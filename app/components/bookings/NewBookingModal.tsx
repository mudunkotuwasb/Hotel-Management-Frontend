  ...
  useEffect(() => {
    if (editingBooking) {
      ...
      setFormData({
        ...
        bedPreference: "Double",
        mealPlan:
          editingBooking.package === "room-only"
            ? "Room Only"
            : editingBooking.package === "bed-breakfast"
            ? "Bed & Breakfast"
            : editingBooking.package === "half-board"
            ? "Half Board"
            : "Full Board",
        specialRequest: "Near pool",
      });
    ...
    try {
      ...
      const bookingData = {
        ...
        booking: {
          ...
          status: "confirmed",
          totalAmount: 141.0 * formData.rooms,
        },
      };
    ...
    try {
      ...
      if (onUpdateBooking) {
        const updatedBooking: Booking = {
          ...
          package:
            formData.mealPlan === "Room Only"
              ? "room-only"
              : formData.mealPlan === "Bed & Breakfast"
              ? "bed-breakfast"
              : formData.mealPlan === "Half Board"
              ? "half-board"
              : "full-board",
        };
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
      <div className="bg-white rounded-lg max-w-3xl w-full max-h-[90vh] overflow-y-auto shadow-xl">
        <div className="p-4 border-b bg-blue-600 hover:bg-blue-700">
                  ...
                  <div>
                    ...
                    <div className="flex">
                      ...
                      <input
                        ...
                        className={`flex-1 border border-l-0 rounded-r px-3 py-2 text-sm text-gray-900 ${
                          fieldErrors.phone
                            ? "border-red-500"
                            : "border-gray-300"
                        }`}
                    ...
                    <p className="text-xs text-gray-500 mt-1">
                      {currentCountry.name}: {currentCountry.phoneDigits} digits
                      required (no spaces)
                    </p>
                  </div>
                </div>
                ...
                <div className="space-y-3">
                  ...
                  <div>
                    ...
                    <div className="space-y-2">
                      {Array.from({ length: formData.rooms }).map(
                        (_, index) => (
                          <select
                            key={index}
                            value={formData.roomNo[index] || ""}
                            onChange={(e) =>
                              handleRoomSelection(e.target.value, index)
                            }
                            className={`w-full border rounded px-3 py-2 text-sm text-gray-900 ${
                              fieldErrors.roomNo
                                ? "border-red-500"
                          ...
                          >
                            ...
                            {availableRooms
                              .filter(
                                (room) =>
                                  !formData.roomNo.includes(room) ||
                                  room === formData.roomNo[index]
                              )
                              .map((room) => (
                      ...
                      {["Single", "Double", "King", "Twin"].map((type) => (
                        <label key={type} className="flex items-center">
                          <input
                            ...
                            onChange={() =>
                              handleInputChange("bedPreference", type)
                            }
                ...
                <div className="space-y-1 text-xs">
                  <div className="flex justify-between">
                    ...
                    <span className="text-gray-900">
                      ${(128.5 * formData.rooms).toFixed(2)}
                    </span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-gray-900">Service Fee:</span>
                    <span className="text-gray-900">
                      ${(12.5 * formData.rooms).toFixed(2)}
                    </span>
                  </div>
                  <div className="flex justify-between">
                  ...
                  <div className="flex justify-between font-bold text-sm border-t border-gray-300 pt-2">
                    ...
                    <span className="text-gray-900">
                      ${(141.0 * formData.rooms).toFixed(2)}
                    </span>
                  </div>
                </div>
              </div>
